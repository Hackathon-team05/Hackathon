...
// ----- ピン設定 -----
static const int BUTTON_PIN        = 4;   // 再生/停止トグル
static const int BUTTON_OCT_UP    = 5;   // 1オクターブ上（押している間）
static const int BUTTON_OCT_DOWN  = 6;   // 1オクターブ下（押している間）
...
// ----- オクターブ設定 -----
static int8_t        octaveOffset     = 0;    // 0 / +12 / -12
...
void setup() {
  ...
  pinMode(BUTTON_OCT_UP,   INPUT_PULLUP);
  pinMode(BUTTON_OCT_DOWN, INPUT_PULLUP);
...
void loop() {
  // オクターブシフト（押している間だけ有効）
  if      (digitalRead(BUTTON_OCT_UP)   == LOW) octaveOffset = +12;
  else if (digitalRead(BUTTON_OCT_DOWN) == LOW) octaveOffset = -12;
  else                                           octaveOffset =   0;
