...
// 拍手間隔が不安定で有効な間隔が2件未満の場合にBPMを更新しないことをテストする。
void test_bpm_generate_rejects_unstable_intervals() {
    // 入力: 100、200、1000、2000ミリ秒の不安定な拍手間隔。
    // 過程: 中央値600ミリ秒の有効範囲420～780から外れる間隔を除外する。
    // 出力: 有効な間隔が0件のため戻り値がfalseで、BPMは既定値70のままになる。
    log_input("拍手間隔=100、200、1000、2000ミリ秒");
    bpm_setup();

    log_process("有効な拍手間隔が2件未満の場合はBPM更新を中止する");
    bpm_generate(0);
    bpm_generate(100);
    bpm_generate(300);
    bpm_generate(1300);
    bool result = bpm_generate(3300);
    ...
    TEST_ASSERT_FALSE(result);
    TEST_ASSERT_EQUAL_INT(CLAP_MAX, clap_count);
    TEST_ASSERT_EQUAL_UINT16(DEF_BPM, bpm);
}
...
void run_bpm_manager_tests() {
    ...
    RUN_TEST(test_bpm_generate_rejects_unstable_intervals);
