...
void i2c_send(int dev, ControlCommand& cmd, InstrumentStatus& status){
    if(dev < 0 || dev >= 4){
        return;
    }
    ...
    delay(1);//楽器のack_okの準備待ち
    
    uint8_t ack_received = Wire.requestFrom(address,(uint8_t)2);
    int j=0;
    while(Wire.available()>0 && j<2){
        status_box[3+j]=Wire.read();//ack_okとchecksumの再読み取り
        j++;
    }
    if(ack_received != 2 || j != 2){
        dev_ctl[dev].error_count++;
        return;
    }
